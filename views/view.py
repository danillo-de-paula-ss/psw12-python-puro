from .__init__ import input_ask
...
class SubscriptionService:
    ...
    def delete(self, id_):
        with Session(self.engine) as session:
            statement = select(Subscription).where(Subscription.id == id_)
    ...
    def pay(self, subscription: Subscription):
        with Session(self.engine) as session:
            ...
            if self._has_pay(results):
                question = input_ask("Essa conta já foi paga esse mês, deseja pagar novamente? Y ou N: ", valid_answers='ynYN')
...
# x = int(input_ask("", valid_answers='01'))
